      ...
      <div className="card-wrapper">
        <div className="card-items">
          {data &&
            data.length > 0 &&
            data.map((item, index) => (
              <div className="cards" key={index} style={{ position: 'relative' }}>
                ...
                <div className={`flex deals-info ${['leaders2', 'leaders3', 'leaders4'].includes(selectedLeader.value) ? 'center-items' : 'justify-between'}`}>
                  {selectedLeader.value === 'leaders' && (
                    <>
                      <div className="flex align-center">
                        <BsCash style={{ fontSize: "25px", color: "#1f7bc1" }} />
                        <span>{item.commission}</span>
                      </div>
                      <div className="flex align-center label-image">
                        <MdLabelOutline style={{ fontSize: "25px", color: "#1f7bc1" }} />
